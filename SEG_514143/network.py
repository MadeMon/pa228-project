...
class ASPP(nn.Module):
    def __init__(self, in_channels, out_channels=256):
        ...
        self.branches = nn.ModuleList([
            nn.Conv2d(in_channels, out_channels, 1),                          # 1x1
            nn.Conv2d(in_channels, out_channels, 3, padding=6, dilation=6),  # rate=6
            nn.Conv2d(in_channels, out_channels, 3, padding=12, dilation=12),# rate=12
            nn.Conv2d(in_channels, out_channels, 3, padding=18, dilation=18),# rate=18
        ])
    ...
    def forward(self, x):
        ...
        out.append(F.interpolate(self.global_pool(x), size=(h, w), mode='bilinear', align_corners=False))
...
class ModelCustom(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.interpolate(features, size=(h, w), mode='bilinear', align_corners=False)